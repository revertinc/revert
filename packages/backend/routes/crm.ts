...
const filterLeadsFromContactsForHubspot = (leads: any[]) => {
  const updatedLeads = leads
    ...
    .filter(
      (lead) =>
        lead.properties?.hs_lead_status === null ||
        lead.properties?.hs_lead_status === undefined
    );
...
crmRouter.get("/oauth-callback", async (req, res) => {
  ...
  try {
    if (integrationId === "hubspot" && req.query.code && req.query.t_id) {
      ...
      // Handle the received code
      const url =
        "https://revert2-dev-ed.develop.my.salesforce.com/services/oauth2/token";
      const formData = {
        grant_type: "authorization_code",
...
crmRouter.post("/leads/search", tenantMiddleware(), async (req, res) => {
  try {
    ...
    if (thirdPartyId === "hubspot") {
      ...
      res.send({
        status: "ok",
        results: filterLeadsFromContactsForHubspot([
          result?.data.results,
        ] as any[]),
      });
    } else if (thirdPartyId === "zohocrm") {
