...
import qs from "qs";
class AuthService {
  async createUserOnClerkUserCreation(
    webhookData: any,
    ...
    if (
      webhookData &&
      ["user.created", "user.updated"].includes(webhookEventType)
    ) {
      try {
        let toUpdate: any = {
          id: webhookData.id,
        ...
        response = await Account.findOneAndUpdate(
          { id: webhookData.id },
          { $set: toUpdate },
          { upsert: true }
        );
  ...
  async saveOAuthDataOnWorkspace(
    workspaceId: string,
        ...
        if (connection.tp_refresh_token) {
          if (connection.tp_id === "hubspot") {
            ...
            const formData = {
              ...
              redirect_uri:
                "https://app.buildwithforest.com/oauth-callback/hubspot",
            ...
            const formData = {
              ...
              redirect_uri:
                "https://app.buildwithforest.com/oauth-callback/zohocrm",
              refresh_token: connection.tp_refresh_token,
            };
            const result = await axios({
              method: "post",
            ...
            const formData = {
              ...
              redirect_uri:
                "https://app.buildwithforest.com/oauth-callback/sfdc",
              refresh_token: connection.tp_refresh_token,
            };
            const result = await axios({
              method: "post",
