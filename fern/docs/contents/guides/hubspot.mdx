
<img src="https://res.cloudinary.com/dfcnic8wq/image/upload/v1705654011/Revert/Docs/st8lxfjo2ljowxbzang1.png"/>


#### Obtaining HubSpot Client ID and Secret

* Open a [HubSpot Developers account](https://app.hubspot.com/signup-hubspot/developers). 
* Go to `Manage Apps`  
* Create a HubSpot app, using the steps mentioned [here](https://legacydocs.hubspot.com/docs/faq/how-do-i-create-an-app-in-hubspot)
  * `NOTE:` You can skip this step and use the default revert Hubspot app
* Set `https://app.revert.dev/oauth-callback/hubspot` as the `redirect` url for your app
* **Get your client\_id and client\_secret**:
  * Go to the "App Credentials" section of the App setting under `Auth`
  * Copy your client\_id and client\_secret

#### Connect to HubSpot via Revert

* Create an account on Revert if you don't already have one. (https://app.revert.dev/sign-up)
* Login to your revert dashboard (https://app.revert.dev/sign-in) and click on `Customize your apps` - `HubSpot`


<img src="https://res.cloudinary.com/dfcnic8wq/image/upload/v1705652583/Revert/Docs/nwlrrom2sw5xj7dsgznv.png"/>


* Enter the `client_id` and `client_secret` you copied in the previous step into the App credentials here and click `Submit`.